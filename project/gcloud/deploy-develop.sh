...
docker build -t gcr.io/${PROJECT_DEV}/${IMAGE_NAME}:dev .
...
yes | gcloud beta container images add-tag gcr.io/${PROJECT_DEV}/${IMAGE_NAME}:dev gcr.io/${PROJECT_DEV}/${IMAGE_NAME}:latest

kubectl set image deployment/${APP_DEPLOYMENT} ${APP_CONTAINER}=gcr.io/${PROJECT_DEV}/${IMAGE_NAME}:dev